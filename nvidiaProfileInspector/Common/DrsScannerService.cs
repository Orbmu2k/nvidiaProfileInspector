...
namespace nvidiaProfileInspector.Common
{
    ...
    public class DrsScannerService : DrsSettingsServiceBase
    {
        ...
        public List<CachedSettings> CachedSettings = new List<CachedSettings>();
        public List<string> ModifiedProfiles = new List<string>();
        public HashSet<string> UserProfiles = new HashSet<string>();
            ...
            ref List<uint> alreadyCheckedSettingIds)
        {
            ...
            if (nvw.Instance.DRS_GetSetting(hSession, hProfile, checkSettingId, ref setting) != NvAPI_Status.NVAPI_OK)
                return false;
        ...
        public async Task ScanProfileSettingsAsync(bool justModified, IProgress<int> progress, CancellationToken token = default(CancellationToken))
        {
            ...
            ModifiedProfiles = ModifiedProfiles.OrderBy(x => x).ToList();
            UserProfiles = new HashSet<string>(UserProfiles.OrderBy(x => x).ToList());
        }
