...
class MessageHelper
{
    ...
    [System.Runtime.InteropServices.DllImportAttribute("User32.dll")]
    static extern bool SetWindowPlacement(int                           hWnd,
        [System.Runtime.InteropServices.InAttribute]ref WINDOWPLACEMENT lpwndpl);
    ...
    {
        var param=new WINDOWPLACEMENT();
        if(MessageHelper.GetWindowPlacement(hWnd,ref param))
        {
            if(param.showCmd!=MessageHelper.SW_NORMAL)
            {
                param.length =System.Runtime.InteropServices.Marshal.SizeOf(typeof(WINDOWPLACEMENT));
                param.showCmd=MessageHelper.SW_NORMAL;
                MessageHelper.SetWindowPlacement(hWnd,ref param);
            }
        }
    ...
    internal int sendWindowsStringMessage(int hWnd,int wParam,string msg)
    {
        ...
        if(hWnd>0)
        {
            ...
            cds.lpData=msg;
            cds.cbData=len+1;
            result    =MessageHelper.SendMessage(hWnd,MessageHelper.WM_COPYDATA,wParam,ref cds);
        }
    ...
    internal int getWindowId(string className,string windowName)=>MessageHelper.FindWindow(className,windowName);
